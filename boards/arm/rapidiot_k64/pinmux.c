...
static int rapidiot_k64_pinmux_init(struct device *dev)
{
	ARG_UNUSED(dev);
	...
	pinmux_pin_set(porte,  7, PORT_PCR_MUX(kPORT_MuxAlt4));
#else
	...
	pinmux_pin_set(porte,  7, PORT_PCR_MUX(kPORT_MuxAsGpio));
#endif
	...
	pinmux_pin_set(porta, 4, PORT_PCR_MUX(kPORT_MuxAsGpio));
        //SW2
	pinmux_pin_set(porte, 9, PORT_PCR_MUX(kPORT_MuxAsGpio));
        //SW3
	pinmux_pin_set(porte, 10, PORT_PCR_MUX(kPORT_MuxAsGpio));
        //SW4
	pinmux_pin_set(porte, 28, PORT_PCR_MUX(kPORT_MuxAsGpio));
...
#if CONFIG_ADC_0
	/* ADC0_SE21 */
	pinmux_pin_set(portd, 13, PORT_PCR_MUX(kPORT_PinDisabledOrAnalog));
#endif
...
SYS_INIT(rapidiot_k64_pinmux_init, PRE_KERNEL_1, CONFIG_PINMUX_INIT_PRIORITY);
